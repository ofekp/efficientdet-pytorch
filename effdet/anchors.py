...
class AnchorLabeler(object):
    ...
    def batch_label_anchors(self, batch_size: int, gt_boxes, gt_classes):
        ...
        for i in range(batch_size):
            ...
            for idx in range(gt_boxes[i].shape[0]):
                assert gt_boxes[i][idx][0] == gt_boxes_i_yxyx[idx][1]
                assert gt_boxes[i][idx][1] == gt_boxes_i_yxyx[idx][0]
                assert gt_boxes[i][idx][2] == gt_boxes_i_yxyx[idx][3]
            ...
            # print("gt_classes[i] [{}]".format(gt_classes[i]))
            # print("cls_targets [{}]".format(cls_targets))
